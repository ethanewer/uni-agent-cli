...
The only production mode is strict mode. There is no silent trusted fallback. cc requires Node 22.19 or newer, matching the current non-workflow baseline, so Disabled mode retains the same runtime support as upstream cc. This release executes workflow source only on macOS: enablement probes a deny-by-default Seatbelt profile around the exact Node executable, then separately probes Node's permission model. Workflow launch stays unavailable unless Seatbelt denies arbitrary filesystem read/write, network, and child-process creation and Node additionally denies filesystem, network, child-process, and worker access. The child is spawned with `shell: false`, an explicit environment containing only `CC_WORKFLOW_SANDBOX=1` plus `OPENSSL_CONF=/dev/null` for deterministic Node startup, and exactly three pipe descriptors. No parent environment, inherited IPC descriptor, project cwd capability, or broker token is present.
...
descriptor and rejects links or special archive entries before lifecycle code
can run; compressed size, expanded size, entry count, per-entry size, and path
length are bounded before extraction. The official Node 22.19.0 npm 10.9.3 CLI
content is digest-pinned for both producer and reproducer. This branch has no
public npm publication path because the unscoped
registry name is not owned.
